...
func makeExtEncoder(
	...
	return func(e *Encoder, v reflect.Value) error {
		...
		//fix by yuandan
		/*
		if len(b) == 2 && b[0] == 145 && b[1] == 192{
			//bin 为空的情况， 无需ExtHeader， 不过要考虑解析的时候(todo)
			return e.write(b)
		}
...
const (
	MessagePackCode_Bin8 = byte(196)
	MessagePackCode_Bin16 = byte(197)
	MessagePackCode_Bin32 = byte(198)
	MessagePackCode_MinFixStr = byte(160)
	MessagePackCode_MaxFixStr = byte(191)
	MessagePackCode_Nil = byte(0xc0)
	MessagePackCode_MinFixArray = 0x90;
)
...
func (d *Decoder) extHeader(c byte) (int8, int, error) {
	...
	extLen := 0
	switch code {
	case MessagePackCode_Bin8:
		extLenByte := byte(0)
		extLenByte,err = d.readCode()
		if err != nil {
			return 0,0,err
		}
		extLen = int(extLenByte)
	case MessagePackCode_Bin16:
		shortByte,err := d.readN(2)
		if err != nil {
			return 0,0,err
		}
		extLen = int( binary.BigEndian.Uint16(shortByte) )
	default:
		intByte,err := d.readN(4)
		if err != nil {
			return 0,0,err
		}
		extLen = int( binary.BigEndian.Uint32(intByte) )
	}

	extID := 1 //hard code for tbuffer
