...
func encodeMapIntintValue(e *Encoder, v reflect.Value) error {
	if v.IsNil() {
		return e.EncodeNil()
	}
	...
	m := v.Convert(mapIntintType).Interface().(map[int]int)
	if e.flags&sortMapKeysFlag != 0 {
		return e.encodeSortedMapIntint(m)
	}

	for mk, mv := range m {
		if err := e.EncodeUint(uint64(mk)); err != nil {
			return err
		}
		if err := e.EncodeUint(uint64(mv)); err != nil {
			return err
		}
	}
...
func (e *Encoder) encodeSortedMapIntint(m map[int]int) error {
	...
	for _, k := range keys {
		err := e.EncodeUint( uint64(k) )
		if err != nil {
			return err
		}
		if err = e.EncodeUint(uint64(m[k])); err != nil {
			return err
		}
	}
