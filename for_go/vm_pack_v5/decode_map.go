...
var (
	...
	mapIntintPtrType = reflect.TypeOf((*map[int]int)(nil))
	mapIntintType    = mapIntintPtrType.Elem()
)
...
func decodeMapIntintValue(d *Decoder, v reflect.Value) error {
	mptr := v.Addr().Convert(mapIntintPtrType).Interface().(*map[int]int)
	return d.decodeMapIntintPtr(mptr)
}

func (d *Decoder) decodeMapIntintPtr(ptr *map[int]int) error {
	...
	m := *ptr
	if m == nil {
		*ptr = make(map[int]int, min(size, maxMapSize))
		m = *ptr
	}
