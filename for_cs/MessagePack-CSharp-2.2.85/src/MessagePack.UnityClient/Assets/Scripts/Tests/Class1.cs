...
namespace SharedData
{
    ...
    //[MessagePackFormatter(typeof(TBuffer.TBufferFormatter))]
    [MessagePackObject]
    public class HttpBasePacket
    {
        ...
        */
        ...
        [Key(3)]
        [MessagePackFormatter(typeof(TBufferFormatter))]
        public TBuffer rawData;
        */
    }
    ...
    {
        public static readonly TBuffer Invalid = new TBuffer { mLength = 0, mBuffer = null };

        public TBuffer(byte[] arr, int length)
        {
            mBuffer = arr;
            mLength = length;
        }
        ...
        public class TBufferFormatter : IMessagePackFormatter<TBuffer>
        {
            public TBuffer Deserialize(ref MessagePackReader reader, MessagePackSerializerOptions options)
            {
                options.Security.DepthStep(ref reader);
                reader.ReadArrayHeader();
                var __buffer__ = reader.ReadBytes()?.ToArray();
                reader.Depth--;

                if (__buffer__ != null)
                {
                    return new TBuffer(__buffer__, __buffer__.Length);
                }
                else
                {
                    return TBuffer.Invalid;
                }
            }

            public void Serialize(ref MessagePackWriter writer, TBuffer value, MessagePackSerializerOptions options)
            {
                writer.WriteArrayHeader(1);
                if (value.IsValid())
                {
                    writer.Write(new ReadOnlySpan<byte>(value.Buffer, 0, value.Length));
                }
                else
                {
                    writer.WriteNil();
